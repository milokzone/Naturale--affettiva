...
class findFaceGetPulse(object):
    ...
    def get_subface_sum(self, coord):
        x, y, w, h = coord
        subframe = self.frame_in[y:y + h, x:x + w, :]
        v1 = np.sum(subframe[:, :, 0])
        v2 = np.sum(subframe[:, :, 1])
        v3 = np.sum(subframe[:, :, 2])

        return (v1 + v2 + v3) / 3.

    def get_exclude_face_means(self):
        coord = (0,0, self.frame_in.shape[0],self.frame_in.shape[1] )
        total_sum = self.get_subface_sum(coord)
        face_sum = self.get_subface_sum( self.face_rect)
        total_frame_pixel = self.frame_in.shape[0]*self.frame_in.shape[1]
        face_frame_pixel = self.face_rect[2]*self.face_rect[3]
        return (total_sum - face_sum)*1.0/(total_frame_pixel-face_frame_pixel)
    ...
    def run(self, cam):
        ...
        # for debugging
        #print( self.frame_in.dtype )

        f_width = self.frame_in.shape[0]
        
        #self.find_faces = False
        if len(self.detected) > 0:
            # use the width/40 as the threshold to cut off small movements
            if self.shift(self.detected) > f_width/40:
                # print('changing face rect')
                self.face_rect = self.detected
        ...
        if self.find_faces:
            ...
            '''
            self.detected = list(self.face_cascade.detectMultiScale(self.gray,
                                                               scaleFactor=1.3,
                                                               minNeighbors=4,
                                                               minSize=(
                                                                   50, 50),
        ...
        # get the mean value of the area which does not contain face
        exclue_face_mean = self.get_exclude_face_means()
        #print('face_mean is %f, exlcude face mean is %f'% (vals, exclue_face_mean))
        # correct for the face area
        vals = vals /exclue_face_mean

        #str_t = str( exclue_face_mean) + "\n"
        #self.f_t.write( str_t )
        #print( vals)
        ...
        if L > 10:
            ...
            # bug fix here to get more adpated to cameras, this tuple could 
            # empty sometime for some camera
            if not len(idx):
                return
